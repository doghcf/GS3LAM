...
class GradSLAMDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        load_stereo: bool = False,  # If True, load stereo images
        **kwargs,
    ):
        super().__init__()
        ...
        if self.load_stereo:
            self.left_color_paths, self.right_color_paths, self.object_paths, self.embedding_paths = self.get_filepaths()
        else: 
