...
class GradSLAMDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if ".png" in object_path:
            ...
            objects_right = np.asarray(imageio.imread(objects_path_right), dtype=float)
...
class GradSLAMDataset2(torch.utils.data.Dataset):
    def __init__(
        self,
        config_dict,
        stride: Optional[int] = 1,
        ...
        **kwargs,
        ...
        self.color_paths, self.color_paths_right, self.depth_paths, self.object_paths, self.embedding_paths = self.get_filepaths()
        
        if len(self.color_paths) != len(self.depth_paths):
            raise ValueError("Number of left color and right color images must be the same.")
        if self.load_embeddings:
            if len(self.color_paths) != len(self.embedding_paths):
                raise ValueError("Mismatch between number of color images and number of embedding files.")
        self.num_imgs = len(self.color_paths)
    ...
    def __getitem__(self, index):
        ...
        depth_path = self.depth_paths[index]
        if ".png" in depth_path:
            depth = np.asarray(imageio.imread(depth_path), dtype=float)
        elif ".npy" in depth_path:
            depth = np.load(depth_path)
        elif ".exr" in depth_path:
            print("Error object format!!!")
